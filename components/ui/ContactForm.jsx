        ...
        () => {
          ...
          emailjs
            .send( serviceID, templateID, {
              ...
              emailPublicKey
            ).then( () => console.log( "Copia enviada al usuario" ));
  ...
  return (
    <div className="w-full max-w-md glass md:px-5 px-2 md:py-10 py-5 rounded-xl">
      <form ref={ form } onSubmit={ sendEmail } className="w-full h-full flex flex-col md:gap-5 gap-2 items-center justify-center">
        ...
        <input required type="email" name="mail" placeholder="Tu email: " className="md:text-lg text-sm w-full md:p-3 p-2 outline-none border-b-2 border-white bg-slate-500 rounded-xl"/>
      ...
      <div
        className={`py-2 text-center text-sm ${ messageSent ? "visible" : "hidden" }`}>
